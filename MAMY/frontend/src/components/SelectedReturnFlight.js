...
export default class SelectedReturnFlight extends Component {
  constructor(props) {
    super(props);
    this.state = {
        flights:[]
  ...
  async componentDidMount() {
    await axios.get('http://localhost:5000/UserHomePage/SelectedFlight/' + window.location.pathname.substring(89,113))
      .then(response => {
...
flightNumber() {
  ...
  return window.location.pathname.substring(88+20,88+24);
} 


flightCabinClass() {
    if(window.location.pathname.substring(114,115)=="1")
    return "Business";
    if(window.location.pathname.substring(114,115)=="2")
    return "First Class";
    if(window.location.pathname.substring(114,115)=="3")
    return "Economy";
    return "";
...
flightBaggageAllowance() {
  ...
  render() {
    return (
        ...
        <Link to={"/UserHomePage/"+window.location.pathname.substring(35,59)} className="navbar-brand">FSR</Link>
        <div className="collpase navbar-collapse">
        <ul className="navbar-nav mr-auto">
        <li><Link to={"/UserHomePage/userSearch/"+window.location.pathname.substring(35,59)} className="nav-link">Search</Link></li>
        </ul>
        </div>
      </nav>
      ...
      <table className="table container">
        ...
        <Link to ={"/UserHomePage/Summary/"+window.location.pathname.substring(35)}> <button>View Flights Summary</button></Link>
